...
# Apply chat template AFTER attaching adapters
...
output_dir = os.path.join(MODELS_PATH, "output_lora_finetuned_gemma-3-1b-it-unsloth-bnb-4bit")
trainer = SFTTrainer(
    model=model,
    tokenizer=tokenizer,
...
finetuned_lora_model_dir = os.path.join(MODELS_PATH,"finetuned_gemma-3-1b-it-unsloth-bnb-4bit-lora")
model.save_pretrained(finetuned_lora_model_dir)
tokenizer.save_pretrained(finetuned_lora_model_dir)

finetuned_lora_model_dir_16_bit = os.path.join(MODELS_PATH,"finetuned_gemma-3-1b-it-unsloth-bnb-4bit-lora-16bit")
